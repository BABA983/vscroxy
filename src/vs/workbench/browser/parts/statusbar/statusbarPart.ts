...
class StatusbarPart extends Part implements IStatusbarEntryContainer {
	...
	override updateStyles(): void {
		...
		const backgroundColor = this.getColor(styleOverride?.background ?? STATUS_BAR_NO_FOLDER_BACKGROUND) || '';
		...
		const foregroundColor = this.getColor(styleOverride?.foreground ?? STATUS_BAR_NO_FOLDER_FOREGROUND) || '';
		...
		const borderColor = this.getColor(styleOverride?.border ?? STATUS_BAR_NO_FOLDER_BORDER) || this.getColor(contrastBorder);
		if (borderColor) {
...
export class MainStatusbarPart extends StatusbarPart {
	...
	constructor(
		...
		super(Parts.STATUSBAR_PART, instantiationService, themeService, storageService, layoutService, contextMenuService, contextKeyService);
	}
}
