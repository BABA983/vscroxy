...
export class PaneCompositePartService extends Disposable implements IPaneCompositePartService {
	...
	constructor(
		...
		this.onDidPaneCompositeClose = Event.any(...ViewContainerLocations.map(loc => Event.map(this.paneCompositeParts.get(loc)?.onDidPaneCompositeClose || Event.None, composite => { return { composite, viewContainerLocation: loc }; }, eventDisposables)));
	}
