...
//#region Whistle Process

export const IWhistleProcessService = createDecorator<IWhistleProcessService>('whistleProcessService');

export interface IWhistleProcessService extends IRemoteService {
...
class WhistleProcessRemoteServiceStub<T extends object> extends RemoteServiceStub<T> {
	constructor(channelName: string, options: IRemoteServiceWithChannelClientOptions<T> | IRemoteServiceWithProxyOptions | undefined, @IWhistleProcessService ipcService: IWhistleProcessService, @IInstantiationService instantiationService: IInstantiationService) {
		super(channelName, options, ipcService, instantiationService);
	}
}

export function registerWhistleProcessRemoteService<T>(id: ServiceIdentifier<T>, channelName: string, options?: IRemoteServiceWithChannelClientOptions<T> | IRemoteServiceWithProxyOptions): void {
	registerSingleton(id, new SyncDescriptor(WhistleProcessRemoteServiceStub, [channelName, options], true));
}
